...
object GOGConstants {
    ...
    val defaultGOGGamesPath: String
        get() {
            val context = appContext ?: throw IllegalStateException("GOGConstants not initialized. Call init() first.")
            val storeDefaultUri = if (PrefManager.useSingleDownloadFolder) PrefManager.defaultDownloadFolder else PrefManager.gogDownloadFolder
            if (storeDefaultUri.isNotEmpty()) {
                ...
                if (baseDir != null) {
                    Timber.i("GOG using user-defined default storage: $baseDir")
                    File(baseDir).mkdirs()
                    return baseDir
                }
            }
    ...
    fun getSanitizedGameFolderName(gameTitle: String): String {
        val sanitizedTitle = gameTitle.replace(Regex("[^a-zA-Z0-9 \\-_]"), "").trim()
        return sanitizedTitle.ifEmpty { "Game" }
    }

    fun getGameInstallPath(gameTitle: String): String {
        return Paths.get(defaultGOGGamesPath, getSanitizedGameFolderName(gameTitle)).toString()
    }
}
